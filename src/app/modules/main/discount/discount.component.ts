import { Component, AfterViewInit, NgZone, OnDestroy, ViewChild, ElementRef, AfterViewChecked, OnInit } from '@angular/core';
...
export class DiscountComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  private resizeCanvas(canvas: HTMLCanvasElement, forceInit = false) {
    ...
    if (forceInit || newWidth !== this.lastCanvasWidth || newHeight !== this.lastCanvasHeight) {
  ...
  private animate = () => {
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      if (dist < this.LINE_DISTANCE * 1.1) {
        ...
        this.ctx.globalAlpha = 0.22 + 0.25 * (1 - dist / (this.LINE_DISTANCE * 1.1));
        this.ctx.beginPath();
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      if (dist < this.MOUSE_REPEL_RADIUS && dist > 0.1) {
        const force = (this.MOUSE_REPEL_RADIUS - dist) / this.MOUSE_REPEL_RADIUS * this.MOUSE_REPEL_STRENGTH;
        p.vx += (dx / dist) * force;
        p.vy += (dy / dist) * force;
      }
      p.x += p.vx;
